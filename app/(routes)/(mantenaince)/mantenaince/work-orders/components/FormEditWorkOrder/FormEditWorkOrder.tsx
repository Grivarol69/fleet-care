...
import { formSchema } from "./FormEditWorkOrder.form";
import {
...
export function FormEditWorkOrder({
  isOpen,
  setIsOpen,
  onEditWorkOrder,
  item,
  ...
  const [refreshWorkOrder, setRefreshWorkOrder] = useState(0);
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      creationMileage: Number(item.creationMileage),
  ...
  // Función para refrescar los datos de la WorkOrder
  const refreshWorkOrderData = async () => {
    try {
      const response = await axios.get(
        `/api/mantenaince/work-order/${item.id}`
      );
      if (response.data) {
        onEditWorkOrder(response.data);
      }
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Actualizar Orden de Trabajo</DialogTitle>
        </DialogHeader>
        <div>
          <Tabs>
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="details">Detalles Orden Trabajo</TabsTrigger>
              <TabsTrigger value="itemsOt">Items Orden Trabajo</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="itemsOt" className="mt-4">
              <div className="min-h-[600px]">
                <ItemWorkOrdersList
                  workOrderId={item.id}
                  onItemsChange={refreshWorkOrderData}
                />
              </div>
            </TabsContent>
          </Tabs>
